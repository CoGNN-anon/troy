#include "TroyFHEWrapper.cuh"

#include "LinearHelper.cuh"
...
public:
    ...
    FHEUtil(size_t polyModulusDegree, vector<int> qs, uint64_t dataBound, uint64_t plainModulus, uint64_t scale, bool isPriv = true);
    ~FHEUtil() {
        // delete keygen;
        // delete encryptor;
        // delete decryptor;
...
FHEUtil::FHEUtil(size_t polyModulusDegree, vector<int> qs, uint64_t dataBound, uint64_t plainModulus, uint64_t scale, bool isPriv) {
...
std::vector<uint64_t> FHEUtil::mul_scaler_and_subtract_random(std::string& cipher, uint64_t scaler, size_t dim) {
    istringstream sin(cipher);
    Ciphertext enc;
    enc.load(sin, *context);
    auto mod = parms.plainModulus().value();
    scaler = scaler % mod;
    Plaintext encoded_scaler;
    encoder->encodePolynomial(std::vector<uint64_t>(1, scaler), encoded_scaler);
    vector<uint64_t> random = randomVector(dim, mod);
    Plaintext encoded_random;
    encoder->encodePolynomial(random, encoded_random);
    evaluator->multiplyPlainInplace(enc, encoded_scaler);
    evaluator->subPlainInplace(enc, encoded_random);
    ostringstream sout;
    enc.save(sout);
    cipher = sout.str();
...
std::vector<uint64_t> FHEUtil::add_and_mul_scaler_and_subtract_random(std::string& cipher, const std::vector<uint64_t>& addend, uint64_t scaler, size_t dim) {
    istringstream sin(cipher);
    Ciphertext enc;
    enc.load(sin, *context);
    auto mod = parms.plainModulus().value();
    assert(addend.size() == dim);
    Plaintext encoded_addend;
    encoder->encodePolynomial(addend, encoded_addend);
    scaler = scaler % mod;
    Plaintext encoded_scaler;
    encoder->encodePolynomial(std::vector<uint64_t>(1, scaler), encoded_scaler);
    vector<uint64_t> random = randomVector(dim, mod);
    Plaintext encoded_random;
    encoder->encodePolynomial(random, encoded_random);
    evaluator->addPlainInplace(enc, encoded_addend);
    evaluator->multiplyPlainInplace(enc, encoded_scaler);
    evaluator->subPlainInplace(enc, encoded_random);
    ostringstream sout;
    enc.save(sout);
...
FHEWrapper::FHEWrapper(bool isPriv) {
    KernelProvider::initialize();
    uint64_t dataBound = (1ul << 44);
    uint64_t mod = (1ul << 44);
    uint64_t scaler = (1ul << 8);
    this->fheUtil = (void*)(new FHEUtil(8192, {60, 60, 60}, dataBound, mod, scaler, isPriv));
...
std::vector<uint64_t> FHEWrapper::mul_scaler_and_subtract_random(std::string& cipher, uint64_t scaler, size_t dim) {
    KernelProvider::initialize();
    return ((FHEUtil*)this->fheUtil)->mul_scaler_and_subtract_random(cipher, scaler, dim);    
}

std::vector<uint64_t> FHEWrapper::add_and_mul_scaler_and_subtract_random(std::string& cipher, const std::vector<uint64_t>& addend, uint64_t scaler, size_t dim) {
    KernelProvider::initialize();
    return ((FHEUtil*)this->fheUtil)->add_and_mul_scaler_and_subtract_random(cipher, addend, scaler, dim);   
}
